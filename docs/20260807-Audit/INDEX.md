...
| --- | --- | --- | --- |
| COLLECTYX-SEC-01 | Critical | S | Escape user data in dashboard.js before interpolating it into innerHTML |
| COLLECTYX-SEC-02 | Critical | S | Add a Content-Security-Policy meta tag so the web build has a CSP |
| COLLECTYX-SEC-03 | Critical | L | Restore deletes everything before validating the replacement data |
...
| COLLECTYX-SEC-12 | Medium | M | Repair the test suite: 4 of 6 suites fail and migration-test was never correct |
| COLLECTYX-SEC-13 | Medium | M | Rank shifting is non-atomic and O(n squared) |
| COLLECTYX-SEC-14 | Medium | S | replaceAllTags behaves differently on each backend; SQLite silently destroys tag links |
| COLLECTYX-SEC-15 | Medium | S | Web cache is mutated before the write commits and is not invalidated on failure |
| COLLECTYX-SEC-16 | Medium | S | Owner (Testing) ships an unauthenticated data-scope switch; set_app_meta accepts any key |
